...
(() => {
  ...
  const elements = {
    ...
    count:
      document.getElementById(
        "pergamonListingCount"
      ),

    status:
      document.getElementById(
        "pergamonMarketStatus"
      ),
  ...
  async function loadPergamonListings() {
    ...
    try {
      ...
      const response =
        await fetch(
          `${window.SUPABASE_API_URL}listings?server_id=eq.${PERGAMON_SERVER_ID}&status=eq.active&select=*&order=created_at.desc`,
          {
            method: "GET",
  ...
  async function loadListingImages() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.warn(
          "Pergamon ilan görselleri alınamadı:",
          response.status
        );
  ...
  function showErrorState() {
    ...
    elements.status.textContent =
      "Pergamon ilanları şu anda yüklenemedi.";
