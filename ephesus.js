...
const CATEGORY_NAMES = {
  ...
  async function loadEphesusListings() {
    ...
    try {
      ...
      const response =
        await fetch(
          `${window.SUPABASE_API_URL}listings?server_id=eq.${EPHESUS_SERVER_ID}&status=eq.active&select=*&order=created_at.desc`,
          {
            method: "GET",
  ...
  async function loadListingImages() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.warn(
          "Ephesus ilan görselleri alınamadı:",
          response.status
        );
...
if (
  ...
  function showErrorState() {
    ...
    elements.status.textContent =
      "Ephesus ilanları şu anda yüklenemedi.";
