...
(() => {
  ...
  // ========================================================
  // AKADEMI TEOS AYARLARI
  // ========================================================

  const AKADEMI_TEOS_SERVER_ID = 4;
  ...
  const elements = {
    ...
    count:
      document.getElementById(
        "akademiTeosListingCount"
      ),

    status:
      document.getElementById(
        "akademiTeosMarketStatus"
      ),

    empty:
      document.getElementById(
        "akademiTeosEmptyState"
      ),

    pagination:
      document.getElementById(
        "akademiTeosPagination"
      ),
  ...
  /*
   * Bu modül sadece akademi-teos.html
   * sayfasında çalışır.
   */
  if (!elements.grid) {
    return;
  ...
  async function loadAkademiTeosListings() {
    ...
    try {
      ...
      const response =
        await fetch(
          `${window.SUPABASE_API_URL}listings?server_id=eq.${AKADEMI_TEOS_SERVER_ID}&status=eq.active&select=*&order=created_at.desc`,
          {
            method: "GET",
      ...
      console.error(
        "Akademi Teos ilan yükleme hatası:",
        error
      );
  ...
  async function loadListingImages() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.warn(
          "Akademi Teos ilan görselleri alınamadı:",
          response.status
        );
      ...
      console.error(
        "Akademi Teos ilan görsel hatası:",
        error
      );
    }
  }
  ...
  function renderPagination() {
    ...
    html += `
      <button
        type="button"
        class="akademi-teos-page-button"
  ...
  function setLoadingState() {
    ...
    elements.status.textContent =
      "Akademi Teos ilanları yükleniyor...";
  ...
  function showErrorState() {
    ...
    elements.status.textContent =
      "Akademi Teos ilanları şu anda yüklenemedi.";
